...
class RKTN(Service):
    ...
    def __init__(self, ctx, title, device, movie, desired_audio_language):
        ...
        if title.startswith("http"):
            ...
            if "episodes" in parts:
                # URL: .../episodes/stream/{season-id}[/{episode-id}]
                ep_idx = parts.index("episodes")
                self.title = parts[ep_idx + 2] if len(parts) > ep_idx + 2 else parts[-1]
                # Direct episode URL: inject --wanted into the framework so only that episode downloads
                if len(parts) > ep_idx + 3 and not ctx.parent.params.get("wanted"):
                    try:
                        season_slug = parts[ep_idx + 2]   # e.g. "sanctuary-1"
                        episode_slug = parts[ep_idx + 3]  # e.g. "sanctuary-1-3"
                        season_num = int(season_slug.split("-")[-1])
                        ep_num = int(episode_slug.split("-")[-1])
                        ctx.parent.params["wanted"] = [f"{season_num}x{ep_num}"]
        ...
        self.classification_id = 18
        self.locale = "en_GB"
        self.market_code = "uk"
        self.session_uuid = ""
        self.access_token = ""
        self.ifa_subscriber_id = ""
        self.hdr_type = "NONE"
        self.kind = "avod"
        self.available_hdr_types = []
    ...
    def get_tracks(self, title: Title_T) -> Tracks:
        ...
        if not self.audio_languages:
            ep_data = getattr(title, 'data', None)
            view_options = (ep_data or {}).get("view_options") if ep_data else None
            if view_options:
                try:
                    streams = view_options["private"]["streams"]
                    all_langs = []
                    for s in streams:
                        for a in s.get("audio_languages", []):
                            if a["abbr"] not in all_langs:
                                all_langs.append(a["abbr"])
                    selected = [l for l in self.desired_audio_language.split(",") if l.strip() in all_langs]
                    self.audio_languages = selected or all_langs[:3]
                except (KeyError, TypeError):
                    pass
            if not self.audio_languages:
                self.audio_languages = [lang.strip() for lang in self.desired_audio_language.split(",") if lang.strip()]
