...
class PBS(Service):
    ...
    def _fetch_video_bridge(self, video_slug: str) -> dict:
        ...
        portal_id = self._resolve_tp_media_id(video_slug) or video_slug
        rp = self.session.get(
            self.config["endpoints"]["portalplayer"] + portal_id + "/",
            params=pp_params,
        )
    ...
    def _parse_video_bridge(self, html: str) -> dict:
        idx = html.index("window.videoBridge = ")
        json_start = html.index("{", idx)
        video_bridge, _ = json.JSONDecoder().raw_decode(html, json_start)
        title_m = re.search(r"<title>\s*Video:\s*(.*?)\s*\|\s*Watch", html, re.DOTALL)
        if title_m:
            video_bridge["_page_title"] = " ".join(title_m.group(1).split())
        return video_bridge

    def _resolve_tp_media_id(self, video_slug: str) -> Optional[str]:
        """Try to find a portalplayer-compatible ID for the slug (legacy fallback)."""
        try:
            r = self.session.get(f"https://www.pbs.org/video/{video_slug}/", timeout=15)
            if not r.ok:
                return None
            m = re.search(r'portalplayer/([A-Za-z0-9]{5,})', r.text)
            if m:
                return m.group(1)
        except Exception:
            pass
